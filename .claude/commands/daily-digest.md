...
6. **Send Notification (REQUIRED)**
   ...
   ```bash
   osascript -e 'tell application "Messages" to send "[SB] Daily digest ready: obsidian://open?vault=Second%20Brain&file=Second%20Brain%2FReports%2FDaily-Digest-{YYYYMMDD}" to participant "{recipient}" of account id (id of 1st account whose service type = iMessage)'
   ```
